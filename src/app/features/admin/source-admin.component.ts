...
@Component({
  ...
  styles: [`
    ...
    * {
    ...
    /* Predictor logo tile. The supplied artwork is a wordmark on white with
       generous padding, so it is shown whole on a white ground rather than
       cropped to fill, which reduced wide marks to an unreadable sliver. */
    .site-tile {
    ...
    .site-tile.has-art { background: #fff; padding: 3px; }

    /* Full-width row inside the two-column settings grid. */
    .settings-grid .settings-card.span-2 { grid-column: 1 / -1; }
    .site-tile img { width: 100%; height: 100%; object-fit: contain; }

    /* Selects were falling through the .fields rule and rendering as raw
       browser controls beside their labels. */
    .fields select {
      box-sizing: border-box;
      width: 100%;
      margin-top: 6px;
